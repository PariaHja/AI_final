...
EPISODES = 1000
class Agent:
    def __init__(self,actions): #initializing an Agent object
        self.alpha = 0.1 #step size
        self.gamma = 0.85
        self.epsilon = 0.01 #introduces randomness to the algo. forcing the agent to try different actions
        # actions which agent can do
        self.action_space = ['u', 'd', 'l', 'r'] #agent can move up, down, left and right
        self.n_actions = len(self.action_space) #number of actions
        self.actions = actions
        self.q_table = defaultdict(lambda : [0.0,0.0,0.0,0.0]) #defaultdict provides a default value for the key that does not exists
    
    def get_action(self, state):
        #next_action = np.random.choice(self.actions)
        if np.random.uniform() > self.epsilon :
            #choosing the best action
            action_values = self.q_table[str(state)] #state must have a value for every 4 variable in q_table
            ...
            next_action = np.random.choice(argmax_actions) 
             ...
             next_action = np.random.choice(self.actions)
        if self.epsilon > 0 :
            self.epsilon -= 0.00001 #making random moves easier to be chosen
        if self.epsilon < 0 : #epsilon can not be negative
            self.epsilon = 0
        return next_action
